...
DIMENSION_PREFIXES = {
    "municipal_policy": (
        "women_", "institutional_", "campaigns_", "programs_",
        "human_rights_", "specialized_women_",
    ),
...
NONNEGATIVE_PREFIXES = (
    "police_", "rate_", "cnes_", "social_", "justice_tjpa_", "transport_",
)
...
SUPPORT_SUFFIXES = (
    "_service_id", "_display_name", "_query", "_note", "_status", "_confidence",
)
...
def finalize_dataset_v2(
    ...
    removed_columns = [
        ...
        if c.startswith(("justice_dpe_", "justice_mppa_"))
        or c in DEPRECATED_JUSTICE_COLUMNS
    ]
    if removed_columns:
    ...
    constant_columns = [
        c for c in df.columns
        if c not in {"municipality_code", "municipality"}
    ...
    for column in df.columns:
        ...
        if column.startswith("protection_network_"):
            validate_nonnegative = (
                column not in COORDINATE_COLUMNS
                and not column.endswith(SUPPORT_SUFFIXES)
            )
    ...
    for column in df.columns:
        ...
        if column in {"municipality_code", "municipality"}:
            ...
            role, eligible, reason = "audit_or_support", False, "location, provenance or identifier field"
            ...
            role, eligible, reason = "candidate_numeric", True, "observed and varying numeric indicator"
    ...
    for name, prefixes in DIMENSION_PREFIXES.items():
        ...
        dimensions[name] = {
            ...
            "mean_cell_completeness": float(df[observed].notna().mean().mean()) if observed else 0.0,
        }
    ...
    if not any(c.startswith("transport_") for c in df.columns):
        warnings.append("Transport source layer exists, but municipal transport indicators are not yet integrated")
    ...
    readiness_status_path.write_text(json.dumps({
        ...
        "transport_indicators_pending": not any(c.startswith("transport_") for c in df.columns),
        "next_step": "Conceptual and statistical screening of candidate criteria",
    }, ensure_ascii=False, indent=2), encoding="utf-8")
    final_status_path.write_text(json.dumps({
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "status": "ready_for_criterion_screening" if not errors else "not_ready",
        "audit_status": status,
        "rows": int(len(df)),
